...
#    cur.execute('''CREATE TABLE mail (http_status int, locator text PRIMARY KEY, message blob)''')
    cur.execute('''CREATE TABLE mail (http_status int, locator text, message blob)''')
...
while True:
    ...
    if tic == skip:
        ...
        fof_ttl += fofcount
        print("{0} processed, {1} errors (ttl), {2:.2f} messages/s, {3} non-200s".format(
...
print("{0} messages archived, {1} errors logged and {2} non-200s returned in {3} seconds".format(
                                      count, errs, fof_ttl, (timef()-start_t)))
fl.close()
dbconn.close()
